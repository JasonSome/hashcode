...
int main(void)
{
	...
	for(a = 0;a < V;a++){
		for(b = 0;b < C;b++){
			...
			Q.push({((double)v_profit[a][b])/V_SIZE[a],a,b});
		}
	}
	//pare to kalytero
	while(!Q.empty()){
		...
		|| tmp.profit != ((double)v_profit[tmp.vid][tmp.cache])/V_SIZE[tmp.vid]
		...
		for(a = 0;a < P_reqs[tmp.vid].size();a++){//for all ends
				for(b = 0;b < P_lat[P_reqs[tmp.vid][a].end].size();b++){
					m = (old_ds[P_reqs[tmp.vid][a].end] - P_reqs[tmp.vid][a].ds)*P_reqs[tmp.vid][a].req;
					v_profit[tmp.vid][P_lat[P_reqs[tmp.vid][a].end][b].cache] -= m;
					if(m > 0)
						up_cache[P_lat[P_reqs[tmp.vid][a].end][b].cache] = 1;
				}
		}
		for(a = 0;a < C;a++){
			if(up_cache[a] == 1)
				Q.push({((double)v_profit[tmp.vid][a])/V_SIZE[tmp.vid],tmp.vid,a});
		}
	}
	c = 0;
	for(a = 0;a < C;a++){
